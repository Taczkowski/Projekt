<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="./style.css">
    <link rel="stylesheet" href="./transition.css">
    <script src="./transition.js" defer></script>
    <link rel="stylesheet" href="falling-ingredients.css">
    <title>P Diddy Pizza</title>
</head>
<body>
<div class="initial-animation-container">
        <img id="left" src="./assets/left.png" alt="">
        <img id="right" src="./assets/right.png" alt="">
        <img id="leftbottom" src="./assets/leftbottom.png" alt="">
        <img id="lefttop" src="./assets/lefttop.png" alt="">
        <img id="rightbottom" src="./assets/rightbottom.png" alt="">
        <img id="righttop" src="./assets/righttop.png" alt="">
      </div>
    <div class="snowflakes"></div>
        <header>
            <div class="logo-container">
        <h1>P.DIDDY PIZZA</h1>
            </div>
    </header>
    <main>
        <div>
            <section>
                <h2>Witamy w P Diddy Pizza!!!</h2>
                <article>
                    <p>Zaloguj się by zamówić najfreakowszą pizze na świecie!!!!!</p>
                </article>
                <div>
                    <input id="login" type="button" class="image-button-login" value="Zaloguj">
                    <input id="continue" type="button" value="Kontynuuj Bez Logowania" class="image-button-register">
                </div>
                <article>
                    <p>Firma P Diddy Pizza nie ponosi odpowiedzialności za dziecięcy olejek w pizzy.</p>
                </article>
            </section>
        </div>
    </main>
    <script src="falling-ingredients.js"></script>
    <script src="./script.js" defer></script>
    <div class="container">
        <img src="./assets/lefttop.png" alt="Left Top" class="box lefttop">
        <img src="./assets/left.png" alt="Left" class="box left">
        <img src="./assets/leftbottom.png" alt="Left Bottom" class="box leftbottom">
        <img src="./assets/righttop.png" alt="Right Top" class="box righttop">
        <img src="./assets/right.png" alt="Right" class="box right">
        <img src="./assets/rightbottom.png" alt="Right Bottom" class="box rightbottom">
    </div>
    <div id="explosion-container"></div>
</body>
</html>
